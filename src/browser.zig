...
pub fn runBrowser() !void {
    ...
    // Disable cache - get the web context and set cache model to NONE
    const context = c.webkit_web_view_get_context(@ptrCast(web_view));
    c.webkit_web_context_set_cache_model(context, c.WEBKIT_CACHE_MODEL_DOCUMENT_VIEWER);
